...
st.caption(f"Ask me anything about our blog articles · Backend: {BACKEND_URL}")
...
with st.sidebar:
    ...
    if st.button("⚙️ Re-index Blogs", use_container_width=True):
    ...
    st.divider()
    st.caption("After adding new blogs, click Re-index or run:\n`python scripts/ingest.py`")
...
if prompt := st.chat_input("Ask about our blogs..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching blogs and generating answer..."):
            try:
                history = [
